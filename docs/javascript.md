...
A small "arrow keys" hint (`#kbdHint`, injected sitewide by `_layouts/default.html`) fades in about 900ms after load, fades back out the moment the visitor actually presses ArrowLeft/ArrowRight, and fades back in on any upward scroll — a proxy for "still reading, might want to jump back" — rather than only getting one chance to notice it before it's gone for good. It only ever shows on a page that has `[data-kbd-stop]` elements, so it never advertises a shortcut that wouldn't do anything there. Hidden entirely under 900px width and on coarse/no-hover (touch) input via CSS media queries, since physical arrow keys aren't the relevant input there regardless of viewport width.

While visible, the keycap cluster itself (`.kbd-hint-keys`, not the outer `.kbd-hint` pill) plays a gentle infinite bounce (`kbdHintBounce`, translateY 0↔4px) so it registers as "here's a thing you can press" rather than just static chrome — the same idea as the home hero's `.scroll-hint` mouse-scroll indicator. The bounce lives on an inner element deliberately: the outer `.kbd-hint` already owns `transform` for its own fade/slide-in transition, and stacking a second, looping animation on that same property on the same element would fight it exactly the way the hero buttons' entrance animation once silently blocked their hover lift (see the `hero-intro-sequence` note above). Skipped under `prefers-reduced-motion: reduce`, same convention as the hero's own bounce.
